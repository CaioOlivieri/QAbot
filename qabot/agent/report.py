...
def _section_reconciliation(reconciliation: dict[str, object]) -> str:
    ...
    lines += [
        ...
        "Escapes are anchored by bug-introducing-commit provenance — the SZZ algorithm "
        "(Śliwerski, Zimmermann & Zeller, MSR 2005) — falling back to a report-time "
        "proxy when git cannot resolve it. Trailing window; confounders (bad-fix "
        "injection, inherited defects) not modeled._",
    ]
    return "\n".join(lines)
