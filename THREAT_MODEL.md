...
| **Secret hygiene** — API keys leak via tracked files | `.env.keys` is never tracked; `.gitignore` blocks `.env*`, `*.key`, `*.pem`, and `reports/*` — with one deliberate exception: the defect ledger `reports/qabot_state.json` is versioned (the nightly trend commit). It carries findings metadata only (file, line, category, severity, description), never secrets; note that finding descriptions can quote analyzed source, so keep public dogfooding limited to public code. | `.gitignore` |
